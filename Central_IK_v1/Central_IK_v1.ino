...
void forwardkin(int SW, int Lone, int Ltwo)
  {
  if (SW == Lone == Ltwo == 0)
    {
      ...
      ledcWrite(pwm_swivel, 0);
      ledcWrite(pwm_link1, 0);
      ledcWrite(pwm_link2, 0);
     }
  else if (SW < 0  && Lone < 0 && Ltwo < 0)
    {
      digitalWrite(dir_swivel, LOW);
      digitalWrite(dir_link1, LOW);
      digitalWrite(dir_link2, LOW);
      int s = map(abs(SW), 0, 11, 0, 200);
      int l1 = map(abs(Lone), 0, 10, 0, 255);
      int l2 = map(abs(Ltwo), 0, 11, 0, 255);
      ledcWrite(pwm_swivel, s);
      ledcWrite(pwm_link1, l1);
      ledcWrite(pwm_link2, l2);
    }
  else if (SW > 0 && Lone > 0 && Ltwo > 0)
    {
  ...
  if (SerialPort.available())
  {
      ...
      else
    {
      if ((changeMode == 1) || (changeMode == 0))
      {
       //ik.Stop();
        forwardkin(Y, Z, X);
        Serial.println("FORWARDDDDDDDDDDDD");
        gripper(grip);
        changeMode = 2;
         }
        drive.MotorCode(x, y, M);
