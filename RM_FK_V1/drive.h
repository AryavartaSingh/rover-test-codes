class Drive
{
  ...
  public:
    ...
    void MotorCode(int x, int y, int M)
    {
      ...
      // FORWARD MAX
      else if (abs(x) < 70 && y > 70)
      {
       Serial.println("FM");

        digitalWrite(Ldir, HIGH);
        digitalWrite(Rdir, HIGH);
        int i = map(abs(y) * (M * 0.1), 70, 1023, 0, 255);
        int j = map(abs(y) * (M * 0.1), 70, 1023, 0, 255);
      ...
      // SPOT LEFT
      else if (x < 70 && abs(y) <= 70)
      {
       Serial.println("SL");

        digitalWrite(Ldir, LOW);
        digitalWrite(Rdir, HIGH);
        int i = map(abs(x) * (M * 0.1), 70, 1123, 0, 255);
        int j = map(abs(x) * (M * 0.1), 70, 1123, 0, 255);
        ledcWrite(Lchannel, i);
        ledcWrite(Rchannel, j);
      }
...
//        Serial.println("SR");
      ...
      // OCTET 1
      else if (x > 70 && y > 70 && x > y)
      {
//        Serial.println("O1");

        digitalWrite(Ldir, HIGH);
        digitalWrite(Rdir, LOW);
        int i = map(abs(abs(x) - abs(y)) * (M * 0.1), 70, 1123, 0, 255);
        int j = map(abs(x) * (M * 0.1), 70, 1123, 0, 255);
        ledcWrite(Lchannel, i);
        ledcWrite(Rchannel, j);
      }
...
//        Serial.println("O3");
        ...
        digitalWrite(Rdir, HIGH);
        int i = map(abs(y) * (M * 0.1), 70, 1023, 0, 255);
        int j = map(abs(abs(x) - abs(y)) * (M * 0.1), 70, 1023, 0, 255);
        ledcWrite(Lchannel, i);
        ledcWrite(Rchannel, j);
      }
...
//        Serial.println("O7");
        ...
        digitalWrite(Ldir, LOW);
        digitalWrite(Rdir, LOW);
        int i = map(abs(abs(x) - abs(y)) * (M * 0.1), 70, 1023, 0, 255);
        int j = map(abs(y) * (M * 0.1), 70, 1023, 0, 255);
        ledcWrite(Lchannel, i);
        ledcWrite(Rchannel, j);
      }
